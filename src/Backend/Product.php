...
class Product
{
    ...
    public function generateDownloadItems(DataContainer $dc)
    {
        ...
        $files = deserialize($dc->activeRecord->uploadedFiles);
    ...
    public function generateTags(DataContainer &$dc)
    {
        ...
        if (isset($dc->activeRecord->tag) && !in_array('tag', $fieldsForTags)) {
    ...
    {
        foreach (deserialize($dc->activeRecord->uploadedFiles) as $file) {
            ...
            if (in_array($extension, explode(',', Config::get('validImageTypes')))) {
     ...
     *
     * @param DataContainer $dc
     * @param boolean       $deleteOnlyGenerated
     */
